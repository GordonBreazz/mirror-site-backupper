...
if ! run_db_backup; then
    log_error "Дамп баз данных завершился с ошибкой — бэкап прерван."
    exit 1
fi
log "Дамп баз данных завершён" "END"

log "Копирование корня сайтов, включая дампы БД (rsync поверх того же соединения)" "START"
if ! run_files_download; then
    log_error "Копирование файлов завершилось с ошибкой — бэкап прерван."
    exit 1
fi
